...
# ═══════════════════════════════════════════════════
_INDIA_TERMS = {
    "india","indian","rbi","rupee","sebi","bse","nse","nifty",
    "sensex","modi","dalal","mumbai","delhi","bengaluru","gujarat",
    "maharashtra","ipo","mutual fund","sip","inr","₹",
}
_INDIA_STOCK_TERMS = {
    "reliance","tcs","infosys","wipro","hdfc","icici","axis bank",
    "sbi","bajaj","airtel","adani","tata","mahindra","maruti",
    "ongc","coal india","bhel","hindalco","ntpc","itc",
}
_INTL_STOCK_TERMS = {
    "apple","microsoft","nvidia","tesla","google","alphabet","amazon",
    "meta","jpmorgan","goldman","dow jones","s&p","nasdaq","sp500",
    "nyse","stock","shares","equity","earnings","ipo","etf",
    "quarterly","revenue","profit","dividend",
}
_GLOBAL_EXCL_INDIA = {
...
def _categorize(title: str, summary: str) -> list[str]:
    ...
    if has_india:
        cats.append("india")
    if has_global_excl and not (has_india and not has_global_excl):
        if not (has_india and not has_intl_stock and not has_global_excl):
            cats.append("global")
    return cats
...
def ticker():
    ...
    for symbol, label in TICKER_SYMBOLS:
        try:
            ...
            results.append({"name": label, "price": f"{price:,.2f}", "change": change_str, "up": is_up})
        except Exception as e:
            print(f"⚠️ Ticker error {symbol}: {e}")
    return results
...
def market():
    try:
        ...
        return {"sensex": f"{n_price:,.2f}", "sp500": f"{sp_price:,.2f}",
                "sensex_change": n_chg, "sp_change": sp_chg,
                "sensex_up": n_up, "sp_up": sp_up}
    except Exception as e:
        print(f"❌ Market error: {e}")
        return {"sensex": "22,519.40", "sp500": "5,214.08",
                "sensex_change": "▲ +112.30  (+0.50%)", "sp_change": "▲ +22.40  (+0.43%)",
                "sensex_up": True, "sp_up": True}


@app.get("/news/carousel")
def carousel():
    return [{"title": a["title"], "label": a["label"], "bias": a["bias"], "source": a["source"]} for a in NEWS_CACHE]
...
def news(topic: str = "all"):
    ...
    return NEWS_CACHE if topic == "all" else [a for a in NEWS_CACHE if topic in a.get("categories", [])]


@app.get("/status")
def status():
    return {"cached_articles": len(NEWS_CACHE), "last_updated": LAST_UPDATED,
